<template>
  <LayTableInfo>
    ...
    <template #content>
      <a-table :columns="ShortLinkColumns"
  ...
  >
    ...
    <a-form ref="dataForm" :model="drawerData.dataInfo" :footer="null" v-bind="formItemLayout"  @cancel="doCancel" >
      <a-form-item has-feedback label="短链接标识" name="identifier"  extra="短链接地址是： /go/">
        <a-input v-model:value="drawerData.dataInfo.identifier" placeholder="请输入短链接标识"></a-input>
      </a-form-item>
      <a-form-item has-feedback label="源链接地址" name="url">
        <a-input v-model:value="drawerData.dataInfo.url" placeholder="请输入源链接地址"></a-input>
      </a-form-item>
...
import {ShortLinkColumns} from "@/config/tableConfigs/qy_short_link";
import {FormInstance, message, notification} from "ant-design-vue";
import {Add, Delete, List, Update,ShortLinkType} from "@/api/short_link";
...
function doAdd() {
  ...
  drawerData.title = "新增短连接"
  drawerData.shortUrl = "新增短连接"
  drawerData.dataInfo = {
    id:"",
    url:"",
    name: "",
...
function doUpdate(data:ShortLinkType) {
  drawerData.visible = true;
  drawerData.title = "编辑短连接【"+data.name+"】";
  drawerData.dataInfo = {
    id:data.id,
    url:data.url,
    name: data.name,
