          ...
          <a-card  class="header-card"  :style="{backgroundImage:'url(' + postData.defImg + ')'}"  v-realBackgroundImg="postData.thumbnail">
            <a-row class="header-wrap">
              ...
              <span class="meta-category">
                 <router-link :to=" {name:'category-name',  params:{name:postData.category.identifier} }"    rel="category tag"> {{postData.category.name}} </router-link>
              </span>
...
const doSetTitleMenuls = () => {
  ...
  console.log("onMounted",titles.length)
  if (!titles.length) {
    dataInfo.titles = [];
    return;
  }
